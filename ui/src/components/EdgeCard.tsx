...
import { MdSwapHoriz, MdAddCircle } from "react-icons/md";
...
// Helper to get layover information
const getLayoverInfo = (transport: Transport) => {
  ...
  for (let i = 0; i < segments.length - 1; i++) {
    ...
    const arrivalTime = currentSeg.arrivalTime?.toDate();
    const departureTime = nextSeg.departureTime?.toDate();

    if (arrivalTime && departureTime) {
      const layoverTime = departureTime.getTime() - arrivalTime.getTime();
      const hours = Math.floor(layoverTime / (1000 * 60 * 60));
      const minutes = Math.floor(
        (layoverTime % (1000 * 60 * 60)) / (1000 * 60),
      );

      layovers.push({
        airportCode:
          currentSeg.arrivalAirportCode || nextSeg.departureAirportCode,
  ...
  return {
    layoverCount: flight.layoverCount || segments.length - 1,
    layovers,
    totalDuration: flight.totalDuration || "",
  };
};

// Helper to get ancillary costs display (non-baggage only)
const getAncillaryCosts = (transport: Transport) => {
  ...
  // Filter out baggage-related costs since they're handled by BaggageDisplay
  return ancillaries.filter(
    (ancillary: any) =>
...
export const EdgeCard = ({
  ...
  const currentTransport =
    options.length > 0 ? options[selectedOptionIndex] : transport;
  ...
  const getSortDetails = () => {
    ...
    const origin = capitalizeFirstLetter(
      currentTransport.originLocation?.city || "Origin",
    );
    const dest = capitalizeFirstLetter(
      currentTransport.destinationLocation?.city || "Dest",
    );
    const layoverInfo = getLayoverInfo(currentTransport);
    ...
    return (
      <VStack align="start" spacing={0}>
        <HStack>
          ...
          {layoverInfo && layoverInfo.layoverCount > 0 && (
            <Badge colorScheme="orange" variant="subtle" fontSize="xs">
              {layoverInfo.layoverCount} layover
              {layoverInfo.layoverCount > 1 ? "s" : ""}
            </Badge>
          )}
        </HStack>
        {timing && (
        ...
        {currentTransport.details.case === "flight" &&
          (() => {
            const flight = currentTransport.details.value;
            const segments = flight.segments || [];
            const layoverInfo = getLayoverInfo(currentTransport);

            // Only show detailed segments if there are layovers
            if (
              segments.length <= 1 ||
              !layoverInfo ||
              layoverInfo.layoverCount === 0
            ) {
              return null;
            }
  ...
  const getTitle = () => {
    if (
      currentTransport.type === TransportType.FLIGHT &&
      currentTransport.details.case === "flight"
    ) {
      const f = currentTransport.details.value;
      return (
  ...
  const baseCost = currentTransport.cost?.value
    ? `${currentTransport.cost.value.toFixed(2)} ${currentTransport.cost.currency}`
    : undefined;
  const totalCostDisplay = totalCost?.value
    ? `${totalCost.value.toFixed(2)} ${totalCost.currency}`
    : baseCost;
  ...
  return (
    <TimelineCard
      ...
      rightContent={<BaggageSummary transport={currentTransport} />}
      tags={currentTransport.tags}
      hideToggle={
        options.length <= 1 || (!!ancillaryCosts && ancillaryCosts.length > 0)
      }
    >
              ...
              {ancillaryCosts.map((ancillary: any, idx: number) => (
                <HStack key={idx} justify="space-between" width="100%">
                  ...
                  <Text fontSize="sm" color="yellow.200" fontWeight="semibold">
                    +{ancillary.cost?.value?.toFixed(2)}{" "}
                    {ancillary.cost?.currency}
                  </Text>
                </HStack>
              ))}
          ...
          <>
            ...
            <Box>
              <HStack mb={2}>
                ...
                <Text fontWeight="semibold" color="gray.300" fontSize="s">
                  Available Options
                </Text>
              </HStack>
                ...
                {options.map((opt, idx) => {
                  ...
                  if (opt.details.case === "flight") {
                    ...
                    if (f.departureTime && f.arrivalTime) {
                      const diff =
                        f.arrivalTime.seconds - f.departureTime.seconds;
                  ...
                  // Get ancillary costs for this option (non-baggage)
                  const optAncillaries =
                    opt.details.case === "flight"
                      ? getAncillaryCosts(opt)
                      : null;
                  const optTotalCost =
                    opt.details.case === "flight" ? getTotalCost(opt) : null;
                  const optPrice = optTotalCost?.value
                    ? `${optTotalCost.value.toFixed(2)} ${optTotalCost.currency}`
                    : opt.cost?.value
                      ? `${opt.cost.value.toFixed(2)} ${opt.cost.currency}`
                      : "";
                      ...
                      <HStack justify="space-between">
                        <VStack align="start" spacing={0}>
                          <Text fontWeight="bold" fontSize="md" color="white">
                            {opt.details.case === "flight"
                              ? `${opt.details.value.carrierCode} ${opt.details.value.flightNumber}`
                              : opt.plugin}
                          </Text>
                          <Text fontSize="sm" color="gray.400">
                            {timing}
                          ...
                          {/* Baggage summary in options */}

                          {optAncillaries && optAncillaries.length > 0 && (
                            <Wrap>
                              {optAncillaries.map(
                                (anc: any, ancIdx: number) => (
                                  <WrapItem key={ancIdx}>
                                    <Badge
                                      colorScheme="orange"
                                      variant="subtle"
